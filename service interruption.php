...
function removeRow(index,url){
	var check=confirm("Remove Record?");
	if(check){
		makeajax("processing.php?removeInterruption="+index,"reloadPage");	
	}
	self.location=url;
...
if(isset($_GET['incident'])){
	...
	$sql="select * from service_interruption where incident_id='".$_GET['incident']."' order by time";
	$rs=$db->query($sql);
...
?>	
		<tr>
			...
			<td><a href='#' onclick='removeRow("<?php echo $row['id']; ?>","service interruption.php?incident=<?php echo $_GET['incident']; ?>")'>X</a></td>
		</tr>
<?php	
	}	
}
